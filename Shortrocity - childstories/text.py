...
def get_text_narrations(text_narration_folder_path)->list[str]:
    """
    Get the text narrations from the data.json file.
    """
    
    # get the name of the json file in the text_narration_folder_path
    json_file_name = [f for f in os.listdir(text_narration_folder_path) if f.endswith('.json')][0]
    print(f'\n\n {json_file_name=}\n {text_narration_folder_path=}');
    print('\n ============\n\n');
    
    with open(os.path.join(text_narration_folder_path,json_file_name)) as f:
        narration_data = json.load(f) 
    narrations = []
    for element in narration_data['elements']:
        if element['type'] == 'text':
            narrations.append(element['content'])
    return narrations

def add_narration_only_to_video(input_video, output_video_path, narration_folder_path, text_narration_folder_path, frame_rate:int=30):
    """
    Add audio narration to a video.
    """
    cap = cv2.VideoCapture(input_video)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    temp_video_path = "capted_output_video.mp4"
    ...
    narrations = get_text_narrations(text_narration_folder_path)
    for i, narration in enumerate(narrations):
        narration_path = os.path.join(narration_folder_path,f"narration_{i+1}.mp3")
        ...
        for _ in range(narration_frames_duration):
            ret, frame = cap.read()
            if not ret:
                break
...
def add_narration_to_video(input_video, output_video_path, narration_folder_path, text_narration_folder_path, frame_rate:int=30):
    """
